...
const News = (props) => {
    ...
    const  updateNews = async ()=> {
        ...
        const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=f9248e44a8c948428e5ffc25bf46ffad&page=${page}&pagesize=${props.pageSize}`
    ...
    useEffect(() => {
        document.title = `${capitalizeFirst(props.category)} - News App`;
        updateNews();
    ...
    const fetchMoreData = async () => {
        ...
        const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}&apiKey=f9248e44a8c948428e5ffc25bf46ffad&page=${page+1}&pagesize=${props.pageSize}`
        setPage(page+1)
        let data = await fetch(url);
        let parseData = await data.json()
        console.log(parseData)
    ...
    // }
        ...
        return (
            <>
                <h1 className='text-center' style={{margin: '35px 0px', marginTop: '62px'}}>Top Headlines </h1> <h3 className='animate-charcter text-center' style={{marginTop: '-10px'}}>{props.title}</h3>
                {loading && <Spinner />}
                ...
                {/* <div className="container d-flex justify-content-between">
                    ...
                    <button disabled={this.state.page + 1 > Math.ceil(props.pageSize)} type="button" className="btn btn-dark" onClick={this.handleNextClick}>Next &gt;</button>
                </div> */}
            </>
        )
}
